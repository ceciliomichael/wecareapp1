...
class HelperSubscriptionScreen extends StatefulWidget {
  ...
  const HelperSubscriptionScreen({super.key, required this.helper});
...
class _HelperSubscriptionScreenState extends State<HelperSubscriptionScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final results = await Future.wait([
        SubscriptionService.getUserSubscription(widget.helper.id),
        SubscriptionService.getActivePlans(),
        SubscriptionService.getUserSubscriptionHistory(widget.helper.id),
        TrialUsageService.getTrialStatus(widget.helper.id),
      ]);
            ...
            content: StatefulBuilder(
              builder:
                  (context, setDialogState) => Column(
                    ...
                    children: [
                      Text(
                        'Plan: ${plan.name} - ${plan.formattedPrice}/${plan.type.shortPeriodLabel}',
  ...
  Future<void> _processPurchase(
    ...
    try {
      ...
      if (mounted) {
        ...
        // Reload data to update history and trial status
        await _loadData();
      }
    } catch (e) {
      if (mounted) {
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
              'Continue using WeCare with unlimited access to job opportunities and premium features',
            ...
            child: Column(
              ...
              children: [
                ...
                _buildFeatureComparison(
                  'App Usage',
                  '${widget.helper.trialLimit} times only',
                  'Unlimited',
                ),
                _buildFeatureComparison('Job Applications', 'Limited', 'Unlimited'),
                _buildFeatureComparison('Priority Applications', '✗', '✓'),
                _buildFeatureComparison('Profile Visibility', 'Basic', 'Enhanced'),
                _buildFeatureComparison('Support', 'Email only', 'Email & Chat'),
                _buildFeatureComparison('Advanced Matching', '✗', '✓'),
              ],
            ),
          ),
        ),
  ...
  Widget _buildFeatureComparison(String feature, String trial, String premium) {
